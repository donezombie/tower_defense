...
from tower_defense.tower_shotgun import *
from tower_defense.tower_aoe import *
...
class Player(GameObject):
    ...
    def build_hole(self,typeTower,cost):
        ...
        for game_object in collide_list:
            if type(game_object) == Hole and game_object.builded == False:
                if tower_defense.game_object.coin >= cost :
                    tower_defense.game_object.coin -= cost
                    game_object.deactivate()
                    game_object.builded = True
                    position_hole = game_object.x, game_object.y
                    new_tower = typeTower(position_hole[0],position_hole[1])
                    add(new_tower)
    ...
    def move(self):
        ...
        if global_input_manager.x_pressed:
            self.build_hole(Tower,tower_ice)
        if global_input_manager.c_pressed:
            self.build_hole(TowerShotgun,tower_flame)
        if global_input_manager.z_pressed:
            self.build_hole(TowerAOE,tower_aoe)
